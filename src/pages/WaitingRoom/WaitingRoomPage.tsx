...
export const WaitingRoomPage = () => {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (result.status === 'NOT_FOUND') {
          console.warn('[WaitingRoom] NOT_FOUND: 대기열 미등록');
          return;
        }

        // WAITING 상태 → aheadCount로 순번 계산
        const newRank = isNaN(ahead) ? null : ahead + 1; // 내 앞 N명 → 나는 N+1번째
    ...
    const startPolling = () => {
      // 1. interval을 먼저 등록하여 intervalRef.current를 확실히 확보
